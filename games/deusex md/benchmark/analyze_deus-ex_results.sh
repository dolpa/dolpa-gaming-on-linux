...
SYSTEM_NAME="${DEUS_EX_MD_SYSTEM_NAME:-${SYSTEM_NAME:-$SYSTEM_NAME_DEFAULT}}"
...
SYSTEM_CONFIG_OVERRIDE_FILE="${DEUS_EX_MD_BENCHMARK_CONFIG:-}"
DEUS_EX_MD_PROTON_VERSION_DEFAULT="GE-Proton10-25"

RESULTS_DIR="${SCRIPT_DIR}/results"
PROFILES_DIR="${SCRIPT_DIR}/profiles"
TESTS_CONFIG_FILE="${SCRIPT_DIR}/config/tests.conf.sh"
GROUPS_CONFIG_FILE="${SCRIPT_DIR}/config/groups.conf.sh"
TEMPLATE_FILE="${RESULTS_DIR}/deus_ex_md_benchmark_report_template.md"
LATEST_REPORT_FILE="${RESULTS_DIR}/deus_ex_md_benchmark_report.md"
TIMESTAMPED_REPORT_FILE="${RESULTS_DIR}/deus_ex_md_benchmark_report_$(date +%Y%m%d_%H%M%S).md"
...
if [[ -n "$SYSTEM_CONFIG_OVERRIDE_FILE" ]]; then
	if [[ ! -f "$SYSTEM_CONFIG_OVERRIDE_FILE" ]]; then
		echo "Error: DEUS_EX_MD_BENCHMARK_CONFIG file not found: $SYSTEM_CONFIG_OVERRIDE_FILE" >&2
		exit 1
	fi
	# shellcheck source=/dev/null
...
# 1) DEUS_EX_MD_PROTON_VERSION (game-specific override)
# 2) PROTON_VERSION (shared system default)
# 3) DEUS_EX_MD_PROTON_VERSION_DEFAULT (game fallback)
PROTON_VERSION="${DEUS_EX_MD_PROTON_VERSION:-${PROTON_VERSION:-$DEUS_EX_MD_PROTON_VERSION_DEFAULT}}"
...
show_help() {
	echo "Deus Ex MD Benchmark Results Analyzer"
...
def parse_json_file(file_path):
	...
	mn = to_float(data.get("minFps"))
	avg = to_float(data.get("averageFps"))
	mx = to_float(data.get("maxFps"))
	if mn is not None and avg is not None and mx is not None:
		return mn, avg, mx
	return None
...
def parse_csv_file(file_path):
	with open(file_path, "r", encoding="utf-8", newline="") as handle:
		...
		normalized = {k: normalize(k) for k in keys}

		min_key = next((k for k, n in normalized.items() if ("min" in n) and ("fps" in n or n == "minimum")), None)
		avg_key = next((k for k, n in normalized.items() if ("avg" in n or "average" in n) and ("fps" in n or "average" in n)), None)
		max_key = next((k for k, n in normalized.items() if ("max" in n) and ("fps" in n or n == "maximum")), None)

		if min_key and avg_key and max_key:
			for row in reversed(rows):
				mn = to_float(row.get(min_key))
				avg = to_float(row.get(avg_key))
				mx = to_float(row.get(max_key))
				if mn is not None and avg is not None and mx is not None:
					return mn, avg, mx

		for row in reversed(rows):
			numeric_values = [to_float(v) for v in row.values()]
...
ensure_test_results_section_exists() {
...

	{
		...
		echo
		echo "- [benchmark/results/deusex-md_benchmark_report_template.md](benchmark/results/deusex-md_benchmark_report_template.md)"
		echo "- [benchmark/results/deusex-md_benchmark_report.md](benchmark/results/deusex-md_benchmark_report.md)"
		echo
		echo "Historical snapshot reports (auto-updated by \\`benchmark/analyze_deus-ex_results.sh\\`):"
		echo
...
register_all_snapshot_report_links_in_readme() {
	...
	shopt -s nullglob
	snapshot_files=("${RESULTS_DIR}"/deusex-md_benchmark_report_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]_[0-9][0-9][0-9][0-9][0-9][0-9].md)
	shopt -u nullglob
...
shopt -s nullglob
result_files=("${RESULTS_DIR}"/*_result_*.json "${RESULTS_DIR}"/*_result_*.csv)
shopt -u nullglob

for result_file in "${result_files[@]}"; do
	file_name="$(basename "$result_file")"
	if [[ "$file_name" =~ ^[0-9]+_result_(.+)_([^_]+)_([^_]+)_([^_]+)_([0-9]{8}_[0-9]{6})(_[^.]+)?\.(json|csv)$ ]]; then
	...
	elif [[ "$file_name" =~ ^[0-9]+_result_(.+)_([0-9]{8}_[0-9]{6})(_[^.]+)?\.(json|csv)$ ]]; then
		test_name="${BASH_REMATCH[1]}"
...
write_report_header "$TEMPLATE_FILE" "Deus Ex Mankind Divided Benchmark Report Template" "Template (blank FPS cells)" "$REPORT_GPU_MODELS" "$REPORT_GPU_VRAMS" "$REPORT_GPU_DRIVERS"
total_tests="$(append_template_rows "$TEMPLATE_FILE")"

write_report_header "$LATEST_REPORT_FILE" "Deus Ex Mankind Divided Benchmark Report" "Latest result per test from JSON/CSV files" "$REPORT_GPU_MODELS" "$REPORT_GPU_VRAMS" "$REPORT_GPU_DRIVERS"
