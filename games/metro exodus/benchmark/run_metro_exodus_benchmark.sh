#!/usr/bin/env bash
# ---------------------------------------------------
# Metro Exodus Benchmark on Ubuntu (Steam)
# ---------------------------------------------------
...
METRO_EXODUS_LAUNCH_MODE_DEFAULT="native"
...
NATIVE_PREFERENCES_FILE="${HOME}/.local/share/feral-interactive/Metro Exodus/preferences"
...
# 1) METRO_EXODUS_LAUNCH_MODE (game-specific override)
# 2) METRO_EXODUS_LAUNCH_MODE_DEFAULT (game default)
METRO_EXODUS_LAUNCH_MODE="${METRO_EXODUS_LAUNCH_MODE:-$METRO_EXODUS_LAUNCH_MODE_DEFAULT}"
METRO_EXODUS_LAUNCH_MODE="${METRO_EXODUS_LAUNCH_MODE,,}"

case "$METRO_EXODUS_LAUNCH_MODE" in
    native|proton)
        ;;
    *)
        echo "Error: invalid METRO_EXODUS_LAUNCH_MODE '$METRO_EXODUS_LAUNCH_MODE'. Supported values: native, proton" >&2
        exit 1
        ;;
esac

INITIAL_LAUNCH_MODE="$METRO_EXODUS_LAUNCH_MODE"
for cli_arg in "$@"; do
    case "$cli_arg" in
        --native)
...
for base_test_name in "${!TESTS[@]}"; do
    ...
    for fg_suffix in "fg-dlss" "fg-frs31" "fg"; do
        ...
        fg_preferences_file_metro_exodus="${PROFILES_DIR}/${fg_test_name}${NATIVE_PREFERENCES_PROFILE_SUFFIX}"

        if [[ -f "$fg_preferences_file_metro_exodus" && ! "${TESTS[$fg_test_name]+isset}" ]]; then
...
show_help() {
    echo "Metro Exodus Benchmark Script"
    ...
    echo "  2) METRO_EXODUS_BENCHMARK_CONFIG=/path/to/file.conf.sh (optional override)"
    echo ""
    echo "System selection override:"
    echo "  METRO_EXODUS_SYSTEM_NAME=MY_MACHINE $0 --group native-quick"
    echo ""
    echo "Launch mode override:"
    echo "  METRO_EXODUS_LAUNCH_MODE=native $0 --group native-quick"
    echo "  METRO_EXODUS_LAUNCH_MODE=proton $0 --group proton-quick"
    echo "  METRO_EXODUS_PROTON_VERSION=GE-Proton9-27 $0 --proton --group proton-quick"
    echo ""
    echo "TESTS:"
...
validate_profiles() {
    ...
    if [[ "$METRO_EXODUS_LAUNCH_MODE" == "proton" ]]; then
...
copy_benchmark_result_file() {
    ...
    local -a source_dir_candidates=(
        "$BENCHMARK_RESULTS_SOURCE_DIR"
        "${HOME}/.local/share/feral-interactive/Metro Exodus/SaveData"
        "${CUSTOM_LIBRARY_PATH}/steamapps/compatdata/${GAME_ID}/pfx/drive_c/users/steamuser/Documents/Metro Exodus/"
        "${CUSTOM_LIBRARY_PATH}/steamapps/compatdata/${GAME_ID}/pfx/drive_c/users/steamuser/My Documents/Metro Exodus/"
        "${STEAM_PATH}/steamapps/compatdata/${GAME_ID}/pfx/drive_c/users/steamuser/Documents/Metro Exodus/"
        "${STEAM_PATH}/steamapps/compatdata/${GAME_ID}/pfx/drive_c/users/steamuser/My Documents/Metro Exodus/"
    )
    ...
    latest_native_txt="$(
        for candidate_dir in "${existing_source_dirs[@]}"; do
            find "$candidate_dir" -maxdepth 1 -type f \( -name 'Metro Exodus_benchmarkresults_*.txt' -o -name 'MetroExodus_benchmarkresults_*.txt' -o -name 'METROEXODUS_*.txt' \) ! -name '*frametimes*' ! -name '*feral_*' -printf '%T@ %p\n' 2>/dev/null
        done | awk -v since="$search_since_epoch" 'since <= 0 || $1 >= since' | sort -nr | head -n1 | cut -d' ' -f2-
    )"
...
apply_setting() {
    ...
    local test_name=${8:-""}
    local effective_launch_mode=${9:-${METRO_EXODUS_LAUNCH_MODE:-native}}
    ...
    # Default launch arguments for Metro Exodus benchmark
...
run_bench() {
    ...
    local -a game_dir_candidates=(
        "$STEAM_PATH/steamapps/common/Metro Exodus"
        "$STEAM_ROOT/steamapps/common/Metro Exodus"
        "$CUSTOM_LIBRARY_PATH/steamapps/common/Metro Exodus"
    )
    ...
    if [[ -z "$game_path" ]]; then
        log_to_file error "$log" "Metro Exodus not found in any known location:"
        for candidate_dir in "${game_dir_candidates[@]}"; do
    ...
    local requested_launch_mode="${METRO_EXODUS_LAUNCH_MODE:-native}"
    ...
    local -a native_exe_candidates=(
        "$game_path/MetroExodus"
        "$game_path/MetroExodus.x86_64"
        "$game_path/bin/MetroExodus"
        "$game_path/bin/linux/MetroExodus"
    )
    local candidate_exe
    ...
    if [[ "$launch_mode" == "proton" ]]; then
        ...
        if [[ "$directx12_mode" == "off" ]]; then
            proton_exe_candidates=(
                "$game_path/bin/x64/MetroExodus.exe"
                "$game_path/MetroExodus.exe"
                "$game_path/bin/x64/metroexodus.exe"
                "$game_path/metroexodus.exe"
            )
        else
    ...
    if [[ "$launch_mode" == "proton" ]]; then
        ...
        pkill -f "MetroExodus.exe|metroexodus.exe|metroexodus|CrashReporter" >/dev/null 2>&1 || true
    fi
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --timeout-minutes)
                ...
                ;;
            --native)
                METRO_EXODUS_LAUNCH_MODE="native"
                shift
                ;;
            --proton)
                METRO_EXODUS_LAUNCH_MODE="proton"
                shift
                ;;
            --all)
                run_all=true
    ...
    # Create log file
    local logfile="${SCRIPT_DIR}/logs/metro_exodus_benchmark_${SCRIPT_RUN_TIMESTAMP}.txt"
    echo "Metro Exodus Benchmark – $(date)" >"$logfile"
    echo "Steam Path: $STEAM_PATH" >>"$logfile"
    echo "Launch Mode (requested): $METRO_EXODUS_LAUNCH_MODE" >>"$logfile"
