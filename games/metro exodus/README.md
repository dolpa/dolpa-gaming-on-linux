# Metro Exodus (Linux/Steam Native)

Benchmark automation and profile management for Metro Exodus on Linux.

## What this folder contains

- `benchmark/run_metro_exodus_benchmark.sh`
...
- `benchmark/analyze_metro_exodus_results.sh`
...
2. Script copies profile by launch mode:
  ...
  - native target: `~/.local/share/feral-interactive/Metro Exodus/preferences`
  - proton source: `benchmark/profiles/{TEST_NAME}.preferences.user.reg`
  - proton target: `.../steamapps/compatdata/412020/pfx/user.reg`
...
- Environment variables:
  - `METRO_EXODUS_LAUNCH_MODE=native|proton`
  - `METRO_EXODUS_PROTON_VERSION=<version>`
  - `METRO_EXODUS_PROTON_VERSION_DEFAULT=<version>`

Proton selection precedence:

1. `METRO_EXODUS_PROTON_VERSION`
2. `METRO_EXODUS_PROTON_VERSION_DEFAULT`
3. `PROTON_VERSION`
...
1. Steam Library → right click **Metro Exodus** → **Properties** → **Compatibility**.
...
2. `METRO_EXODUS_BENCHMARK_CONFIG=/path/to/file.conf.sh` (optional runtime override)

System name selection:

- Default: short hostname from `hostname -s`
- Override for current run: `METRO_EXODUS_SYSTEM_NAME=MY_MACHINE`

Example with explicit config file:

- `METRO_EXODUS_BENCHMARK_CONFIG="/path/to/my-machine.conf.sh" games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --group native-quick`

## Reporting flow

`analyze_metro_exodus_results.sh` reads copied benchmark result files and generates:

- `benchmark/results/metro_exodus_benchmark_report_template.md`
- `benchmark/results/metro_exodus_benchmark_report.md`
- `benchmark/results/metro_exodus_benchmark_report_<timestamp>.md`
...
- [benchmark/results/metro_exodus_benchmark_report_template.md](benchmark/results/metro_exodus_benchmark_report_template.md)
- [benchmark/results/metro_exodus_benchmark_report.md](benchmark/results/metro_exodus_benchmark_report.md)

Historical snapshot reports (auto-updated by `benchmark/analyze_metro_exodus_results.sh`):
...
- Validate all profile files:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --validate-profiles`
...
- Show help:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --help`
- List tests:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --list`
- List groups:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --groups`
- Run default test:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh`
- Run one test:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh native-1080p-high-rt-off`
- Run one or more groups:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --group native-quick`
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --group native-1080p-scaling --group native-1440p-scaling`
- Force native launch mode:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --native --group native-quick`
- Force Proton launch mode:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --proton --group proton-quick`
- Force specific Proton version for this run:
  - `METRO_EXODUS_PROTON_VERSION=GE-Proton9-27 games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --proton --group proton-quick`
- Run all tests:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --all`
- Enable GameMode wrapper:
  - `games/metro exodus/benchmark/run_metro_exodus_benchmark.sh --gamemode --group native-quick`

Analyze results:

- Analyze all tests:
  - `games/metro exodus/benchmark/analyze_metro_exodus_results.sh`
- Analyze a group:
  - `games/metro exodus/benchmark/analyze_metro_exodus_results.sh --group native-quick`
- Analyze specific tests:
  - `games/metro exodus/benchmark/analyze_metro_exodus_results.sh native-1080p-low-rt-off native-4k-high-rt-off`

Convert markdown reports to graphical outputs:

- Generate HTML + PNG from one report:
  - `python3 src/convert_markdown_reports.py games/metro exodus/benchmark/results/metro_exodus_benchmark_report.md --format both`
- Generate only PNG charts split by resolution:
  - `python3 src/convert_markdown_reports.py games/metro exodus/benchmark/results/metro_exodus_benchmark_report.md --format png --split-by-resolution`
